#!/usr/bin/env python3
"""
TSN Traffic WebUI - Simple FastAPI Backend
...
async def root():
    ...
    return "<h1>KETI TSN Traffic Tester</h1><p>Landing page not found</p>"
...
async def app_page():
    ...
    if html_file.exists():
        return html_file.read_text()
    return "<h1>Error</h1><p>Application not found</p>"
...
async def handle_message(websocket: WebSocket, message: dict):
    ...
    try:
        ...
        # Server control
            ...
            if server == "iperf3":
                # iperf3 servers are managed externally
                await websocket.send_json({
                    "type": "server_started",
